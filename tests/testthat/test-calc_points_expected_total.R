...
data_table_current <- calc_table_current(results = data_results_zero)
...
data_points_expected_remaining <- calc_points_expected_remaining(games_remaining = data_model_parameters_unplayed)

data_test <- calc_points_expected_total(
...
test_that("calculation correct", {
  expect_equal(
    data_test |> dplyr::filter(midName == "Liverpool") |> dplyr::slice(1) |> dplyr::pull(Exp_Points_Ave),
    89.8,
    tolerance = 0.01
  )
})
